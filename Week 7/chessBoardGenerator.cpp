...
@date: 7/11/2026
@purpose: Generate a chess board of stars with the user specified side length
*/
...
int main(){ //start
    ...
    cout << "Enter the desired side length for the chess board (as an integer):\n"; //prompt the user to enter a side length for the square
    ...
    for(int i = 1; i <= sideLength; i++){ //int i at 1, if i is less than or equal to sideLength, add 1 to i, 
        for(int row = 1; row <= sideLength; row++){// int row at 1, if row is less than or equal to sidelength, add 1 to row,
            if ((row + i) % 2 == 0){ //if the row is even, do:
                cout << "* "; //print a star 
            }else{ //otherwise:
                cout << "  "; //print a space
            }
